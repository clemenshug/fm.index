...
class int_vector
{
  ...
  public:
    ...
    static size_t read_header(int_vector_size_type & size, int_width_type & int_width, std::istream & in)
    {
        ...
        {
            Rcpp::Rcerr << "Warning: Width of int_vector<" << (size_t)t_width << ">";
            Rcpp::Rcerr << " was specified as " << (size_type)read_int_width << std::endl;
            Rcpp::Rcerr << "Length is " << size << " bits" << std::endl;
        }
    ...
    static uint64_t write_header(uint64_t size, uint8_t int_width, std::ostream & out)
    {
        if (t_width > 0)
        {
            ...
            {
                Rcpp::Rcout << "Warning: writing width=" << (size_type)int_width << " != fixed " << (size_type)t_width
                          << std::endl;
            }
        }
