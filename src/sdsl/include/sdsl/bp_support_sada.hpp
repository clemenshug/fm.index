...
//#define USE_CACHE
    ...
    //! Calculate the maximal parentheses \f$ j \leq sml_block_idx\cdot t_sml_blk+(t_sml_blk-1) \f$ with \f$
    //! excess(j)=desired\_excess \f$
    ...
    //! Calculate the minimal parentheses \f$ j \geq sml_block_idx\cdot t_sml_blk \f$ with \f$ excess(j)=desired\_excess
    //! \f$
  ...
  public:
    ...
    size_type find_close(size_type i) const
    {
        assert(i < m_size);
        if (!(*m_bp)[i])
        { // if there is a closing parenthesis at index i return i
            return i;
        }
#ifdef USE_CACHE
        size_type a = 0;
    ...
    size_type find_open(size_type i) const
    {
        assert(i < m_size);
        if ((*m_bp)[i])
        { // if there is a opening parenthesis at index i return i
            return i;
        }
#ifdef USE_CACHE
        size_type a = 0;
    ...
    //! Calculate the index of the opening parenthesis corresponding to the closest matching parenthesis pair enclosing
    //! i.
    ...
    size_type rmq_open(const size_type l, const size_type r) const
    {
        ...
        if ((*m_bp)[res] == 1)
        { // The parenthesis with minimal excess is opening
        ...
        else
        {
            ...
            if (res < r)
            { // The parenthesis with minimal excess if closing and the next opening parenthesis is less than r
                assert((*m_bp)[res] == 1);
    ...
    size_type rmq(size_type l, size_type r) const
    {
        ...
        if (sbl == sbr)
        { // if l and r are in the same small block
        ...
        else
        {
            ...
            size_type temp = median_block_rmq(sbl + 1,
                                              ...
                                              min_ex); // scan the medium block of l
            if (temp != (size_type)-1)
            {
            ...
            if (mbr - mbl > 1)
            {
                ...
                while (rcb < mbr - 1)
                { // go up the tree until the rightmost covered block >= mbr-1
                    if (min_value(v) <= min_ex)
                    {
                        min_ex = min_value(v);
                        min_pos = v;
                ...
                while (rcb != mbr - 1)
                { // go down the tree until the rightmost covered block = mbr-1
            ...
            { // if it contains the minimum return its position
