  ...
  public:
    ~int_vector_mapper()
    {
        if (m_mapped_data)
        {
            ...
            if (ret != 0)
            {
                Rcpp::Rcerr << "int_vector_mapper: error unmapping file mapping'" << m_file_name << "': " << ret
                          << std::endl;
            }
            ...
            { // write was possible
                if (m_data_offset)
                { // if the file is not a plain file
                    ...
                    {
                        Rcpp::Rcerr << "int_vector_mapper: could not open file for header update" << std::endl;
                        /*
        ...
        if (m_fd != -1)
        {
            ...
            if (ret != 0)
            {
                Rcpp::Rcerr << "int_vector_mapper: error closing file mapping'" << m_file_name << "': " << ret
                          << std::endl;
            }
            if (m_delete_on_close)
            {
                ...
                if (ret_code != 0)
                {
                    Rcpp::Rcerr << "int_vector_mapper: error deleting file '" << m_file_name << "': " << ret_code
                              << std::endl;
                }
            }
        }
