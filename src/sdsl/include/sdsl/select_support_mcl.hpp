...
select_support_mcl<t_b, t_pat_len>::select_support_mcl(const bit_vector * f_v)
  : select_support(f_v)
{
  ...
  , m_arg_cnt(ss.m_arg_cnt)
{
    ...
    if (ss.m_longsuperblock != nullptr)
    {
        ...
        for (size_type i = 0; i < sb; ++i) { m_longsuperblock[i] = ss.m_longsuperblock[i]; }
    }
    m_miniblock = nullptr;
    if (ss.m_miniblock != nullptr)
    {
        m_miniblock = new int_vector<0>[sb]; // copy miniblocks
        for (size_type i = 0; i < sb; ++i) { m_miniblock[i] = ss.m_miniblock[i]; }
    }
}
        ...
        if (select_support_trait<t_b, t_pat_len>::found_arg(i, *v))
        {
            ...
            { //
                ...
                {                                                                              // longblock
                    ...
                    m_longsuperblock[sb_cnt] = int_vector<0>(SUPER_BLOCK_SIZE,
                                                             0,
                                                             bits::hi(arg_position[(arg_cnt - 1) % SUPER_BLOCK_SIZE]) +
                                                                                                               1);
...
void select_support_mcl<t_b, t_pat_len>::init_fast(const bit_vector * v)
{
         ...
         i += 64, ++data)
    {
        ...
        {
            arg_position[last_k64 - 1] = i + select_support_trait<t_b, t_pat_len>::ith_arg_pos_in_the_word(
                                                                                               *data,
                    ...
                    for (size_type j = arg_position[0], k = 0;
                        ...
                        if (select_support_trait<t_b, t_pat_len>::found_arg(j, *v))
                        {
                            if (k >= SUPER_BLOCK_SIZE)
                            {
                                ...
                                {
                                    Rcpp::Rcout << "(" << ii << "," << m_longsuperblock[sb_cnt][ii] << ") ";
                                }
                                Rcpp::Rcout << std::endl;
                                Rcpp::Rcout << "k=" << k << " SUPER_BLOCK_SIZE=" << SUPER_BLOCK_SIZE << std::endl;
                                Rcpp::Rcout << "pos_of_last_arg_in_the_block" << pos_of_last_arg_in_the_block
                                          << std::endl;
                                Rcpp::Rcout.flush();
                            }
    ...
    if (last_k64 > 1)
    {
        ...
        {
            if (select_support_trait<t_b, t_pat_len>::found_arg(i, *v)) { m_longsuperblock[sb_cnt][k++] = i; }
        }
        ++sb_cnt;
    }
}
...
inline auto select_support_mcl<t_b, t_pat_len>::select(size_type i) const -> size_type
{
    ...
    if (m_longsuperblock != nullptr and !m_longsuperblock[sb_idx].empty()) { return m_longsuperblock[sb_idx][offset]; }
    else
    {
        if ((offset & 0x3F) == 0)
        {
...
{
    m_arg_cnt = 0;
    if (nullptr == m_v) { m_logn = m_logn2 = m_logn4 = 0; }
    else
    {
                                                  ...
                                                  -> size_type
{
    ...
    {                                                                      // if there exists 1-bits to be supported
        ...
        if (m_longsuperblock != nullptr)
        {
            ...
            for (size_type i = 0; i < sb; ++i) mini_or_long[i] = !m_miniblock[i].empty();
        }
        ...
        structure_tree_node * child_long = structure_tree::add_child(
                                                          child, "longsuperblock", util::class_name(m_longsuperblock));
        structure_tree::add_size(child_long, written_bytes_long);
        structure_tree_node * child_mini = structure_tree::add_child(
                                                          child, "minisuperblock", util::class_name(m_miniblock));
...
void select_support_mcl<t_b, t_pat_len>::load(std::istream & in, const bit_vector * v)
{
    ...
    {                          // if there exists 1-bits to be supported
        ...
        if (!mini_or_long.empty()) m_longsuperblock = new int_vector<0>[sb]; // Create longsuperblock int_vector<0>

        for (size_type i = 0; i < sb; ++i)
            if (!mini_or_long.empty() and not mini_or_long[i]) { m_longsuperblock[i].load(in); }
            else
            {
...
void select_support_mcl<t_b, t_pat_len>::CEREAL_SAVE_FUNCTION_NAME(archive_t & ar) const
{
    ...
    if (m_arg_cnt)
    {
        ...
        for (size_type i = 0; i < sb; ++i)
        {
            if (!mini_or_long.empty() and !mini_or_long[i]) { ar(CEREAL_NVP(m_longsuperblock[i])); }
            else
            {
...
void select_support_mcl<t_b, t_pat_len>::CEREAL_LOAD_FUNCTION_NAME(archive_t & ar)
{
    ...
    if (m_arg_cnt)
    {
        ...
        if (!mini_or_long.empty()) { m_longsuperblock = new int_vector<0>[sb]; }
